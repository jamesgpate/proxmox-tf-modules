...
resource "proxmox_virtual_environment_file" "bootstrap_user_data_cloud_config" {
  ...
  source_raw {
    ...
    runcmd:
        ...
        - curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="${var.k3s_exec}" INSTALL_K3S_VERSION=${var.k3s_version} K3S_TOKEN=${var.join_token} sh -s - server --cluster-init --tls-san=${var.server_hostname}
    EOF
...
resource "proxmox_virtual_environment_file" "server_user_data_cloud_config" {
  ...
  source_raw {
    ...
    runcmd:
        - apt update && apt upgrade -y
        - curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="${var.k3s_exec}" INSTALL_K3S_VERSION=${var.k3s_version} K3S_TOKEN=${var.join_token} sh -s - server --server https://${var.server_hostname}:6443 --tls-san=${var.server_hostname}
    EOF
...
resource "proxmox_virtual_environment_file" "agent_user_data_cloud_config" {
  ...
  source_raw {
    ...
    runcmd:
        - apt update && apt upgrade -y
        - curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="${var.k3s_exec}" INSTALL_K3S_VERSION=${var.k3s_version} K3S_TOKEN=${var.join_token} sh -s - agent --server https://${var.server_hostname}:6443
    EOF
